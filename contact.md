---
layout: single
title: "Contact Me"
permalink: /contact/
author_profile: true
---

## Navigation

- **[About](/about/)** - Learn more about me and my background
- **[Contact](/contact/)** - Get in touch with me
- **[Blog](/blog/)** - Read my latest articles
- **[Social](/social/)** - Connect with me on social media
- **[University](/university/)** - My educational background and achievements
- **[GitHub](/github/)** - My GitHub profile and projects

## Get in Touch

I'm always interested in connecting with fellow developers, discussing technology, or exploring new opportunities. Feel free to reach out through any of the following channels:

### Professional Networks

- **LinkedIn**: [Mario Andres Botero Florez](https://www.linkedin.com/in/mario-andres-botero-florez)
- **GitHub**: [@mariobot](https://github.com/mariobot)

### Content & Writing

- **Blog**: [mariobot.wordpress.com](https://mariobot.wordpress.com)

### What I Can Help With

- **.NET Development**: Architecture, best practices, and implementation
- **Full Stack Development**: End-to-end application development
- **Cloud Technologies**: Azure deployment and configuration
- **Microservices**: Design patterns and architecture
- **Code Reviews**: Happy to provide feedback on open source projects
- **Technical Discussions**: Software engineering topics and emerging technologies

### Based In

📍 Colombia

### Availability

I'm open to:
- Freelance consulting opportunities
- Technical discussions and collaborations
- Speaking engagements
- Open source contributions

---

*Looking forward to connecting with you!*
