---
layout: home
author_profile: true
header:
  overlay_color: "#000"
  overlay_filter: "0.5"
excerpt: "Software Developer specializing in .NET and Full Stack Development"
---

## About Me

I'm Mario Andres Botero Florez, a passionate software developer with expertise in .NET technologies and full stack development. I focus on building robust, scalable applications and contributing to open source projects.

## Technical Skills

- **.NET Development**: C#, ASP.NET Core, .NET Framework
- **Full Stack**: Frontend and Backend development
- **Software Engineering**: Architecture, design patterns, best practices
- **Version Control**: Git, GitHub

## Projects

Explore my work and contributions on [GitHub](https://github.com/mariobot).

## Featured Blog Articles

I write about software development, cloud technologies, and my experiences. Check out some of my most popular articles:

- [Lista de conceptos MICROSERVICIOS](https://mariobot.wordpress.com/2020/08/05/conceptos-microservicios/) - Key concepts and patterns in microservices architecture
- [Configurar un certificado gratis en tu WebApp de Azure](https://mariobot.wordpress.com/2020/04/28/gratis-ssl-webapp-azure/) - How to configure free SSL certificates on Azure Web Apps
- [Mi experiencia con Windows 10](https://mariobot.wordpress.com/2025/09/16/mi-experiencia-con-windows-10/) - My experience and insights on Windows 10

Visit my full blog at [mariobot.wordpress.com](https://mariobot.wordpress.com) for more articles on engineering, development, and technology.

## Connect

Feel free to connect with me on [LinkedIn](https://www.linkedin.com/in/mario-andres-botero-florez), check out my repositories on [GitHub](https://github.com/mariobot), or read my [blog](https://mariobot.wordpress.com).

